...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (labelTextHeight == null || horLabelTextWidth == null) {
                paint.setTextSize(sp2px(getContext(), getGraphViewStyle().getTextSize()));
	...
	protected void drawHorizontalLabels(Canvas canvas, float border,
		...
		for (int i = 0; i < horlabels.length; i++) {
			...
			drawMultilineText(horlabels[i], x, height - labelTextHeight,
					paint, canvas);
		}
	}

	/**
	 * Allow to draw multiline text. Works the same way as canvas.drawText, but
	 * here canvas is passed as last argument.
	 * 
	 * @author mateusz buda
	 * @param str
	 *            String to be drawn (lines splited with "\n")
	 * @param x
	 ...
	 * @param canvas
	 */
	protected void drawMultilineText(String str, float x, float y, Paint paint,
			Canvas canvas) {
		int lineHeight = 0;
		float yoffset = 0;
		...
		for (int i = 0; i < lines.length; ++i) {
			canvas.drawText(lines[i], x, (float) (y + (yoffset)), paint);
	...
	private String[] generateHorlabels(float graphwidth) {
		...
		double min = round(getMinX(false), 2);
		double max = round(getMaxX(false), 2);
